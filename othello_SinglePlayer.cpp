...
public:
    OthelloGame(char user[50])           //constructor to initialize the game
    {
    ...
    void showCurrentTurn(int playerNo,int userNo)
    {
        ...
        if(playerNo==-1)
        {
            ...
            if(playerNo==userNo)
            {
                    outtextxy(x+len+12,y+len/2-1,username);
            }
            else
            {       outtextxy(x+len+12,y+len/2-1,"CPU");
            }
        }
        ...
        else
        {
            ...
            outtextxy(x+len+1,y+len/2-1,arr);
            if(playerNo==userNo)
            {   outtextxy(x+len+10,y+len/2-1,username);
            }
            else
            {       outtextxy(x+len+10,y+len/2-1,"CPU");
            }
        }
...
int main()
{
    ...
    OthelloGame G(username);
    cout<<"\n\t\tOTHELLO GAME \n";
    G.drawBoard();

    int playerTurn,user;

    cout<<"Choose the player as you want to play:"
        <<"\n\t -1: - Black "
        <<"\n\t  1: - White\n";
    cin>>user;

    cout<<"Enter the player to start with:"
        <<"\n\t -1: - Black "
        <<"\n\t  1: - White\n";
    cin>>playerTurn;
    ...
    while(!G.isGameOver())
    {
        ...
        /*if(playerTurn==user)
            ...
            cout<<"CPU'S TURN (O)";
        */
        G.showCurrentTurn(playerTurn,user);
        ...
        if(playerTurn==user)
        {
            ...
            cout<<"\nEnter row and column to make move :";
            cin>>x>>y;
            while(!G.isLegalMove(x-1,y-1,playerTurn))
            {
                cout<<"\nNot a legal Move .Please try again : ";
                cin>>x>>y;
            }*/
